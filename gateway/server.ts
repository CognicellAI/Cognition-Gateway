...
import { executeApprovedDispatchRun } from "./src/lib/gateway/approval-runner";
...
app.prepare().then(async () => {
  ...
  await initCronScheduler({ db, serverUrl, broadcast, scopeUserId: "gateway-automation" });

  const processQueuedDispatchRuns = async (): Promise<void> => {
    const queuedRuns = await db.dispatchRun.findMany({
      where: { status: "queued" },
    ...
    await Promise.all(
      queuedRuns.map((run) =>
        executeApprovedDispatchRun(run.id, {
          db,
          serverUrl,
          broadcast,
        }),
  ...
  const approvalQueueInterval = setInterval(() => {
    processQueuedDispatchRuns().catch((error: unknown) => {
      console.error("[approvals] Failed to process queued runs:", error);
    });
  }, 3000);
  ...
  const gracefulShutdown = async () => {
    ...
    clearInterval(approvalQueueInterval);
